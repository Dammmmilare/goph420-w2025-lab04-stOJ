...
def main():
    ...
    int_5 = np.where(t < 120)[0][-1]

    m5 = np.linspace(-0.15, 0.8, 20)
    n5 = np.zeros_like(m5)

    for i, m5m in enumerate(m5):
        n5[i] = np.count_nonzero(M_data[int_4:int_5] > m5m)

    mask5 = n5 > 0
    y5 = np.log10(n5[mask5])
    m5 = m5[mask5]
    ...
    # Plotting each interval separately
    figures_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../figures"))
    os.makedirs(figures_dir, exist_ok=True)

    data_list = [
        (y_model1, a1, y1, m1, "0hrs < t < 35hrs", "First Time Interval.png"),
        (y_model2, a2, y2, m2, "35hrs < t < 45hrs", "Second Time Interval.png"),
        (y_model3, a3, y3, m3, "45hrs < t < 73hrs", "Third Time Interval.png"),
        (y_model4, a4, y4, m4, "73hrs < t < 96hrs", "Fourth Time Interval.png"),
        (y_model5, a5, y5, m5, "96hrs < t < 120hrs", "Fifth Time Interval.png"),
    ]

    for y_model, a, y_scatter, m_scatter, title, filename in data_list:
        plt.figure(figsize=(6, 5))
        plt.plot(m_scatter, y_model, label=f"y = {a[0]:.2f} + {a[1]:.2f}m", color='blue')
        plt.scatter(m_scatter, y_scatter, color='black')
